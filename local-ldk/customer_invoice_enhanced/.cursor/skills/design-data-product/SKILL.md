...
4. **Ground everything in the docs** — before using any Vulcan concept, syntax, or pattern in output, confirm it against the Vulcan documentation in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` (search your indexed workspace and read the relevant page). For concrete code syntax, also read from `dpbs-docs/vulcan-examples/`. When in doubt, look it up — never rely on memory.
...
- If they use unfamiliar terms, read the relevant page in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` to clarify
...
Read the **model kinds** page in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` now — before any architecture decisions are made.
...
- After each batch, actively look for gaps and open questions yourself — do not wait for the user to volunteer them. Ground every inference in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`.
...
- **Unknown concept**: Be transparent. Tell the user you couldn't find it in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`, ask if it's a custom term.
...
Create two checklists from your `data-product-plan.md`, then read the relevant page(s) in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` for each item (search your indexed workspace) and confirm your understanding:
...
Read the `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` pages for every Vulcan concept above; for business terms, confirm the definition with the user if the docs don't cover them.
...
If NO examples found: tell the user, and fall back to the patterns documented in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`.
...
rule types and YAML structure in the `dq` and audits pages of `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`, and read dq and audits files from `dpbs-docs/vulcan-examples/<Q5b>/` only
...
### Concepts Verified:
- Grain: [definition from dpbs-docs/vulcan-docs and dpbs-docs/vulcan-docs-philosophy]
...
- Model Kind: [chosen kind with rationale from dpbs-docs/vulcan-docs and dpbs-docs/vulcan-docs-philosophy]
