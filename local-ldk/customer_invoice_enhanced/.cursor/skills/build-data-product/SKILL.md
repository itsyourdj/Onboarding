...
- If **missing** → initialize the project using `vulcan init <engine_name>`:
  ...
  **Add `ignore_patterns` to `config.yaml` BEFORE any plan** — this is mandatory. Vulcan scans the entire project directory, so without ignore patterns it will pick up files from `dpbs-docs/` (including `dpbs-docs/vulcan-examples/`, `dpbs-docs/vulcan-docs/`, `dpbs-docs/vulcan-docs-philosophy/`) and attempt to compile them as models, causing spurious errors. Open `config.yaml` and add:
...
The design workflow already verified core Vulcan concepts (grain, measures, metrics, dimensions, entities, model kinds). Read the relevant pages in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` only for implementation-level concepts not covered in the design verification:
...
If the spec has NO verification summary, fall back to full verification: read the `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` pages for every Vulcan concept (grain, MODEL kind, assertions, semantic measures, time dimensions) and confirm every business term.
...
- **Grain**: [one row represents...]
  Source: dpbs-docs/vulcan-docs and dpbs-docs/vulcan-docs-philosophy (grain)
- **Entities**: [list]
...
- Model Kind: [FULL/INCREMENTAL_BY_TIME_RANGE/INCREMENTAL_BY_UNIQUE_KEY/EMBEDDED/SEED/SCD_TYPE_2_BY_TIME/SCD_TYPE_2_BY_COLUMN/VIEW] chosen because [data volume/refresh pattern/source characteristics from dpbs-docs/vulcan-docs and dpbs-docs/vulcan-docs-philosophy]
...
- **Model Kind**: [FULL/INCREMENTAL_BY_TIME_RANGE/INCREMENTAL_BY_UNIQUE_KEY/EMBEDDED/SEED/SCD_TYPE_2_BY_TIME/SCD_TYPE_2_BY_COLUMN/VIEW]
  Rationale: [from dpbs-docs/vulcan-docs and dpbs-docs/vulcan-docs-philosophy]
...
1. **Ground in the docs** — before using any Vulcan concept, syntax, or pattern in output, confirm it against `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` (and read from `dpbs-docs/vulcan-examples/` for syntax). See Resource Selection Quick Reference.
2. **Never reason beyond the docs** — if a concept, syntax, or pattern isn't explicitly covered in `dpbs-docs/vulcan-docs/`, `dpbs-docs/vulcan-docs-philosophy/`, or `dpbs-docs/vulcan-examples/`, don't deduce or extrapolate an answer from general knowledge. Say it's undocumented and ask the user or point to the closest documented alternative. Do NOT search the web or consult dataos.info (or any other online DataOS/Vulcan documentation site) as a fallback — it may not match this project's bundled version. `https://v2.dataos.info` is the one exception — it's the correct, current documentation domain and may be used.
...
2. Look up the error in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` (search for the error text or the concept it touches) to understand the root cause and fix
3. Fix the broken file yourself, cross-checking against the relevant `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` pages and the Vulcan syntax rules below
...
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| Any Vulcan concept mentioned                       | Read the relevant page(s) in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`                                                                 | BEFORE using the concept in any output                                                  |
...
| After generating a file, or when vulcan plan fails | Self-review the file against `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`, the Vulcan syntax rules below, and the group examples          | After writing each file to catch Vulcan-specific issues; and in the error recovery loop |
...
examples from `dpbs-docs/vulcan-examples/` (category: `...`, engine: `<engine>`) and the relevant `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` pages.
...
`dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` and read from `dpbs-docs/vulcan-examples/` (category: `dq` or `audits`, engine: `<engine>`), then
...
2. Generate the file:
   ...
   - After drafting: self-review the file against `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`, the Vulcan syntax rules below, and the group examples — check for Vulcan-specific issues (forbidden keys, measure name collisions, test format errors, unknown ai_context keys). Fix any issues before writing the file to disk.
...
file contents plus the spec. Ground naming conventions and metadata fields in `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/`.
Go file by file:
...
If results look wrong, consult `dpbs-docs/vulcan-docs/` and `dpbs-docs/vulcan-docs-philosophy/` to understand the issue and fix the syntax yourself (cross-check with `dpbs-docs/vulcan-examples/`).
